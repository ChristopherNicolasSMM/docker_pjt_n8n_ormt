...
MAX_CHARS = 6000  # ajuste depois se quiser (ex: 7000)
SPLIT_RE = re.compile(r"\n{2,}")  # separa por parágrafos

def split_text_by_paragraphs(text: str, max_chars: int) -> list[str]:
    paras = SPLIT_RE.split(text.strip())
    parts = []
    buf = ""
    ...
    for p in paras:
        ...
        # se um parágrafo sozinho é maior que max_chars, quebra "na marra"
        if len(p) > max_chars:
            ...
            # quebra esse parágrafo grande em fatias
            start = 0
            while start < len(p):
...
def is_already_split(path: Path) -> bool:
    # detecta "132.1.md", "132.2.md" etc
    return bool(re.search(r"\.\d+\.md$", path.name))

def preprocess_chunks_folder(book_id: str, knowledge_root: str = "knowledge") -> dict:
    folder = Path(knowledge_root) / book_id / "chunks"
    ...
    for md in sorted(folder.glob("*.md")):
        ...
        if len(parts) <= 1:
            # não conseguiu dividir de forma útil
            skipped += 1
            continue

        stem = md.stem  # "132"
        for i, part in enumerate(parts, start=1):
            out = folder / f"{stem}.{i}.md"
            # opcional: adicionar cabeçalho mínimo de rastreio
            content = f"<!-- source: {md.name} part: {i}/{len(parts)} -->\n\n{part}\n"
            out.write_text(content, encoding="utf-8")
            created += 1

        # ao invés de apagar, renomeia (evita ser reindexado)
        bak = folder / f"{md.name}.bak"
        md.rename(bak)
        changed += 1

    return {"ok": True, "files_split": changed, "files_created": created, "files_skipped": skipped}
...
def qdrant_delete_doc_id(qc: QdrantClient, doc_id: str):
    # Remove todos os pontos cujo payload.doc_id == doc_id
    flt = Filter(must=[FieldCondition(key="doc_id", match=MatchValue(value=doc_id))])
    qc.delete(collection_name=QDRANT_COLLECTION, points_selector=flt)
...
#    return book_ids
def list_books_with_chunk_counts():
    KNOWLEDGE_ROOT.mkdir(parents=True, exist_ok=True)
    books = []
    for d in sorted(KNOWLEDGE_ROOT.iterdir(), key=lambda x: x.name.lower()):
        if not d.is_dir():
            continue
        chunks_dir = d / "chunks"
        if not chunks_dir.exists():
            continue
        n = len(list(chunks_dir.glob("*.md")))
        if n > 0:
            books.append({"id": d.name, "chunks": n})
    return books

#@app.get("/ingest")
#def ingest_page():
#    book_ids = list_book_ids_with_chunks()
#    return render_template(
#        "ingest.html",
#        title="Indexar",
#        knowledge_root=str(KNOWLEDGE_ROOT),
...
@app.get("/ingest")
def ingest_page():
    books = list_books_with_chunk_counts()
    return render_template(
        "ingest.html",
        title="Indexar",
        knowledge_root=str(KNOWLEDGE_ROOT),
...
def ingest_run():
    ...
    batch_size = int(request.form.get("batch_size") or 200)
    workers = int(request.form.get("workers") or 2)
    reindex = (request.form.get("reindex") or "") == "1"

    # Sanitiza (slugify já remove coisas estranhas)
    book_id = slugify(book_id_raw)

    #Processa Chunks para evitar truncamento no ingest (ex: chunk de 10k palavras que não gera embedding e trava o processo)
    preprocess_chunks_folder(book_id)
    
    # Hard block
    if any(x in book_id_raw.lower() for x in ["/", "\\", "*.md", "chunks"]):
        flash("book_id inválido. Selecione apenas a pasta (sem /chunks, sem *.md).", "danger")
        return redirect(url_for("ingest_page"))
    ...
    # cria job
    job_id = str(uuid4())
    job_set(job_id, status="queued", book_id=book_id, total=len(files), inserted=0, errors=0, processed=0, message="Na fila")

    th = threading.Thread(
        target=ingest_worker,
        args=(job_id, book_id, batch_size, workers, reindex),
        daemon=True
    )
    th.start()

    flash(f"Ingest iniciado em background (job={job_id}). Você pode acompanhar abaixo.", "success")
    return redirect(url_for("ingest_page", job=job_id))




def ingest_worker(job_id: str, book_id: str, batch_size: int, workers: int, reindex: bool):
    job_set(job_id, status="running", book_id=book_id, inserted=0, errors=0, processed=0, message="Iniciando...")
    t0 = time.time()

    chunks_dir = KNOWLEDGE_ROOT / book_id / "chunks"
    files = sorted(chunks_dir.glob("*.md"))
    total = len(files)
    job_set(job_id, total=total)

    try:
        # Descobrir dimensão do embedding e garantir coleção
        vec = ollama_embeddings("teste", timeout=30)
        dim = len(vec)
        ...
        if reindex:
            job_set(job_id, message="Reindex: limpando doc_id no Qdrant...")
            qdrant_delete_doc_id(qc, book_id)

        job_set(job_id, message="Gerando embeddings e enviando batches...")

    except Exception as e:
        job_set(job_id, status="error", message=f"Falha inicial: {e}")
        return
    ...
    def build_point(path: Path):
        ...
        v = ollama_embeddings(text, timeout=180)
        return PointStruct(id=pid, vector=v, payload=payload)

    def flush(qc: QdrantClient):
        nonlocal inserted, points
        if not points:
            return
        qc.upsert(collection_name=QDRANT_COLLECTION, points=points)
        inserted += len(points)
        points = []
        job_set(job_id, inserted=inserted)
    ...
    try:
        with ThreadPoolExecutor(max_workers=max(1, workers)) as ex:
            ...
            for fut in as_completed(futures):
                try:
                    ...
                    processed += 1
                    if pt is None:
                        job_set(job_id, processed=processed)
                        continue
                    ...
                    errors += 1
                    processed += 1
                    job_set(job_id, errors=errors, processed=processed, message=f"Erro em chunk (continuando): {e}")
        ...
        job_set(job_id, status="error", message=f"Falha no ingest: {e}", inserted=inserted, errors=errors, processed=processed)
